...
const Post = ({post}: {post: AllPostsQueryResult[number]}) => {
  ...
  return <Card image={image} key={_id} title={title} subTitle="Blog" link={post.slug} index={1} />
}

const Posts = ({children, heading}: {children: React.ReactNode; heading?: string}) => (
  <div className="flex flex-col gap-10 items-center">
    {heading && <h2 className="h2">{heading}</h2>}
    <div className="flex flex-wrap -ml-10 items-center justify-center">{children}</div>
  </div>
)
